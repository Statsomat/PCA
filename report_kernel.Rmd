...
```{r, results="asis", eval=eval3}
cat("\n# Correlations variables - dimensions", fill=TRUE)
```
...
```{r, results="asis", eval=eval3}
cat("\n# Contribution variables - dimensions", fill=TRUE)
```
...
```{r, results="asis", eval=eval3}
cat("\n# Quality of representation", fill=TRUE)
corrplot(pca$var$cos2, is.corr=FALSE)
```

```{r, results="asis", eval=eval3}
cat("\n# Total cos2 of variables on Dim.1 and Dim.2", fill=TRUE)
```
