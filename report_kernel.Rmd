...
if (exists("df_cont")){
   if (ncol(df_cont) == ncol(df) && ncol(df_cont)>=3) {
    ...
    cat("**Error: For some selected variables we cannot assume a continuous distribution. The Principal Components Analysis (PCA) may not be a suitable method. There are more appropriate techniques to deal with mixed data types. For support contact: support@statsomat.com**")
    cat("\\newline",fill=TRUE) 
   } else if (ncol(df_cont) < 2) {
...
tryCatch({
  df_norm <- df_cont_complete
  if (length(knnoutlier(df_cont))>0){
    df_norm <- df_cont_complete[-knnoutlier(df_cont),]
  } 
  ...
  if (sum(!qqcors)==0 || pnorm > 0.01){
    ...
    cat("Warning: We cannot assume an approximately (multivariate) normal distribution of the data. Depending on the distribution of the data and the purpose of the analysis, the Principal Components Analysis (PCA) may not be the optimal dimensionality reduction method. Consider transformations or other dimensionality reduction methods i.e. kernel PCA or Independent Component Analysis (ICA), compare to the results delivered by the PCA and choose the best method. For support contact support@statsomat.com", fill=TRUE)
  }
})
...
ncp <- min(ncol(df),5)
pca <-  PCA(df, ncp=ncp, scale.unit = TRUE)
...
cat("First five five observations of the dataset. ", fill=TRUE)
knitr::kable(t(head(df, n=5)), digits=3, linesep = '', longtable = T) %>%
    kable_styling(position = "center", full_width = FALSE, latex_options = c("HOLD_position","repeat_header"))
```

```{r, results="asis", eval=eval2, dev="cairo_pdf", fig.align="center"}
cat("\n## Pearson Correlations (Matrix)", fill=TRUE)
cat("Pearson Correlation. Only complete cases (if any, missings are removed). ", fill=TRUE)
M <- cor(df, use="complete.obs", method="pearson")
corrplot(M, is.corr=TRUE, method="number", type="lower", tl.col="#396e9f", tl.cex = 0.5, cl.cex = 0.5, number.cex = 0.3, cl.align.text="l")
```

```{r, results="asis", eval=eval2, dev="cairo_pdf", fig.align="center"}
cat("\n## Kendall Correlations (Matrix)", fill=TRUE)
cat("Kendall Correlation.  Only complete cases (if any, missings are removed). ", fill=TRUE)
...
if (ncp==3){
  gridExtra::grid.arrange(grobs = list(plot12,plot13,plot23), 
                      nrow=1, ncol=3, widths = unit(c(10, 10, 10), "cm"), padding = unit(0.3, "line")) 
} else if (ncp==4){
  gridExtra::grid.arrange(grobs = list(plot12,plot13,plot23,plot14,plot24,plot34), 
                      nrow=2, ncol=3, widths = unit(c(10, 10, 10), "cm"), padding = unit(0.3, "line")) 
} else {
...
```{r, results="asis", eval=eval2, dev="cairo_pdf", fig.height=5, fig.align="center"}
